...
for sent in sents:
    for m in sent:
        dst = m.dst
        if dst != -1:
            IS_SRC_IN_NOUN = False
            IS_DST_IN_VERB = False
            for mo in m.morphs:
                if mo.pos == "名詞":
                    IS_SRC_IN_NOUN = True
            
            for mo in sent[dst].morphs:
                if mo.pos == "動詞":
                    IS_DST_IN_VERB = True
            
            if IS_SRC_IN_NOUN and IS_DST_IN_VERB:
                print("".join([mo.surface for mo in m.morphs if mo.pos != "記号"]),"\t", "".join([mo.surface for mo in sent[dst].morphs if mo.pos != "記号"]))
